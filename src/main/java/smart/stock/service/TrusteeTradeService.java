...
public class TrusteeTradeService {
        ...
        if(!CollectionUtils.isEmpty(list)){
            for(TrusteeTradeDto dto: list){
                ...
                if(dto.getStatus() == Constants.TrusteeTradeStatus.Sold.getKey()){
                    ...
                    dto.setIncomeRate(dto.getIncome().divide(dto.getTotal(), 3));
                }
            }
        }
        return list;
    ...
    public TrusteeTradeDto confirm(Long id){
        ...
        //份额
        trusteeTradeDto.setUnit(trusteeTradeDto.getTotal().divide(trusteeTradeDto.getUnitPrice(), 3).intValue());
        //状态为已确认
        ...
        if(fund.getPrincipal().compareTo(BigDecimal.ZERO) > 0){
            //重新计算仓位 保留两位,四舍五入
            fund.setPosition(fund.getPrincipal().subtract(fund.getBanlance()).divide(fund.getPrincipal(), 3));
        }else{
    ...
    public TrusteeTradeDto sale(Long id){
        ...
        trusteeTradeDto.setSaleTotal(trusteeTradeDto.getSaleTotal()
                .add(trusteeTradeDto.getTotal()
                        .multiply(new BigDecimal(trusteeTradeDto.getInterestRate()).divide(new BigDecimal(100), 3))
        ...
        trusteeTradeDto.setIncomeRate(trusteeTradeDto.getIncome().divide(trusteeTradeDto.getTotal(), 3));
